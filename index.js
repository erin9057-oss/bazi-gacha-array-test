...
const GameDatabase = [
    { name: "恋与深空", keywords: ["深空"], desc: "五星卡满150抽会送一张自选...", characters: [] },
    { name: "世界之外", keywords: ["世外"], desc: "网易无限流言情手游...", characters: [] },
    { name: "无限暖暖", keywords: ["暖暖"], desc: "抽四星阁/五星阁。四星5抽保底，五星20抽保底...", characters: [] }
];
...
function appendToChatInput(text) {
    ...
    if ($chatInput.length) {
        ...
        toastr.success("✨ GM断语已添加到输入框，详细天机已暗中注入 D1 (阅后即焚)！");
    } else {
...
jQuery(async () => {
    try {
        ...
        toastr.error("玄学排盘加载失败！请检查文件夹名是否为 bazi-gacha-array-test");
        return;
    }
...
async function executeDivination(mode, actionType = null) {
    ...
    try {
        ...
        if (useStApi) {
            // 兼容写法：如果用户旧版 generateRaw 存在就直接用
            const context = SillyTavern.getContext();
            if (context.generateRaw) {
                aiContentString = await context.generateRaw({ systemPrompt: systemPrompt, prompt: userPrompt });
            } else if (window.TavernHelper && window.TavernHelper.generateRaw) {
                aiContentString = await window.TavernHelper.generateRaw({
                    ordered_prompts: [ { role: "system", content: systemPrompt }, { role: "user", content: userPrompt } ]
                });
            } else {
                throw new Error("当前酒馆 API 不支持生成，请在配置中填写自定义 API！");
            }
        } else {
        ...
        // ================== RPG 无痕注入逻辑 ==================
        if (mode === 'rpg') {
            if (aiResult.summary) appendToChatInput(aiResult.summary);
            
            if (aiResult.details) {
                if (window.TavernHelper && window.TavernHelper.injectPrompts) {
                    window.TavernHelper.injectPrompts([{
                        id: "bazi_rpg_inject_" + Date.now(), role: "system",
                        content: `[System Note: ${aiResult.details}]`,
                        position: "in_chat", depth: 1
                    }], { once: true });
                } else {
                    // 保底方案：如果没有酒馆助手，用内置 Slash 命令暴力注入
                    SillyTavern.executeSlashCommandsWithOptions(`/inject id=bazi_rpg_event position=chat depth=1 role=system [System Note: ${aiResult.details}]`);
                }
            }
...
jQuery(async () => {
    try {
        ...
        console.error("❌ 加载 HTML 界面失败，请检查 extensionFolderPath 路径是否正确", e);
        return;
    }
...
async function executeDivination(mode, actionType = null) {
    ...
    if (mode === 'real') {
        ...
        // 【核心修复2】：采用原生规范调用上下文，杜绝 undefiend 报错
    ...
    try {
        ...
        if (useStApi) {
            // 【核心修复3】：严格分清 TavernHelper 的特权 API
            if (window.TavernHelper && window.TavernHelper.generateRaw) {
                ...
                throw new Error("检测到未安装或未启用酒馆助手 (JS-Slash-Runner)！请安装后使用，或取消勾选【使用酒馆当前主 API】采用独立直连模式。");
            }
        } else {
            const response = await fetch(apiUrl, {
                method: 'POST',
