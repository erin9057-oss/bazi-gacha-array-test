...
console.log("🔮 [玄学排盘] 脚本开始解析...");

// ================== [DEBUG] 注释掉所有聊天注入与RPG相关逻辑 ==================
/*
...
// ================== 初始化与 Tab 切换 ==================
jQuery(async () => {
    console.log("🔮 [玄学排盘] jQuery ready, 开始加载界面...");
    try {
        console.log(`🔮 尝试获取: ${extensionFolderPath}/bazi_ui.html`);
        const uiHtml = await $.get(`${extensionFolderPath}/bazi_ui.html`);
        $("#extensions_settings").append(uiHtml);
        console.log("🔮 成功！bazi_ui.html 已挂载到 #extensions_settings");

        console.log(`🔮 尝试获取: ${extensionFolderPath}/bazi_modal.html`);
        const modalHtml = await $.get(`${extensionFolderPath}/bazi_modal.html`);
        $("body").append(modalHtml);
        console.log("🔮 成功！bazi_modal.html 已挂载到 body");
    } catch (e) {
        console.error("❌ 界面加载彻底失败，可能是路径不对或文件缺失:", e);
        return;
    }

    // 事件绑定
    $("#bazi_open_modal_btn").on("click", () => {
        console.log("🔮 点击了打开面板按钮");
...
async function executeDivination(mode, actionType = null) {
    ...
    try {
        ...
        if (useStApi) {
            // ================== [DEBUG] 只保留最保守的 fetch / 外部请求调用 ==================
            // 为了排除酒馆内置 API 挂掉的问题，这部分暂且不调用酒馆的 generateRaw
            // 如果报错，会直接走到 catch 打印日志
            console.log("🔮 [测试] 尝试调用系统内置大模型接口...");
            const context = SillyTavern.getContext ? SillyTavern.getContext() : window;
            if (context.generateRaw) {
                aiContentString = await context.generateRaw({ systemPrompt: systemPrompt, prompt: userPrompt });
            } else {
                throw new Error("酒馆 API 生成函数未找到，建议使用独立 API。");
            }
        } else {
            console.log("🔮 [测试] 发起外部独立 API 请求...");
